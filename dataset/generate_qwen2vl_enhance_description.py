    ...
    for asin in tqdm(asins_with_image, desc='Image2text'):
        ...
        try:
            ...
            asin_to_caption[asin] = {
                'caption': 'Image description is ' + caption,
                'title': title,
                'image_url': url,
            }
...
def generate_ehanced_description_with_vlm(asin_to_image, asin_to_title, asin_to_description, vlm_model_name, device):
    ...
    Return format: {asin: {caption: text description, title: title, image_url: image url}}
    """
    asin_to_enhanced_description: Dict[str, str] = {}
    error_count = 0
    ...
    for asin in tqdm(asins_with_image, desc='Enhanced Description'):
        url = asin_to_image[asin]
        title = asin_to_title[asin]
        ...
        try:
            ...
            with torch.inference_mode():
                out_ids = vlm_model.generate(**inputs, max_new_tokens=128)
            out_ids_trimmed = out_ids[0][len(inputs.input_ids[0]):]
            enhanced_description = processor.decode(out_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False)
            asin_to_enhanced_description[asin] = {
    ...
    total_items = max(1, len(asins_with_image))
    print(f"Enhanced description error count: {error_count}")
    print(f"Enhanced description error rate: {error_count / total_items * 100:.2f}%")

    return asin_to_enhanced_description
...
def main():
    ...
    if os.path.exists(enhanced_description_path):
        ...
        with open(enhanced_description_path, 'w', encoding='utf-8') as f:
            json.dump(asin_to_enhanced_description, f, ensure_ascii=False, indent=2)
        print(f"Saved Enhanced description: {len(asin_to_enhanced_description)} -> {enhanced_description_path}")
    ...
    # Original description embeddings
    generate_and_save_embeddings(
        embedding_dir, model, asin_to_description, {}, {}, batch_size,
        modality='text', desc='Original description', id2item=id2item, filename='original_description'
    )
