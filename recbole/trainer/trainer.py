...
class Trainer(AbstractTrainer):
    ...
    def _safe_torch_load(self, file_path):
        """Load a torch checkpoint compatibly with PyTorch>=2.6 default weights-only behavior.

        Tries default `torch.load` (weights_only=True in 2.6). If it fails with
        UnpicklingError, retries with `weights_only=False` which may execute
        arbitrary code. Only used for trusted local checkpoints saved by this codebase.
        """
        try:
            with warnings.catch_warnings():
                warnings.filterwarnings(
                    "ignore",
                    message="Detected pickle protocol 4 in the checkpoint",
                    category=UserWarning,
                    module=r"torch\._weights_only_unpickler",
                )
                return torch.load(file_path, map_location=self.device)
        except pickle.UnpicklingError:
            return torch.load(file_path, map_location=self.device, weights_only=False)
    ...
    def resume_checkpoint(self, resume_file):
        ...
        checkpoint = self._safe_torch_load(resume_file)
    ...
    def evaluate(
        ...
        if load_best_model:
            ...
            checkpoint = self._safe_torch_load(checkpoint_file)
            self.model.load_state_dict(checkpoint["state_dict"])
            self.model.load_other_parameter(checkpoint.get("other_parameter"))
