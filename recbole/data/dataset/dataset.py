...
class Dataset(torch.utils.data.Dataset):
    ...
    def _split_index_by_leave_one_out(self, grouped_index, leave_one_num):
        ...
        for index in grouped_index:
            ...
            if self.config['aug']:
                ...
                next_index[0].append(index[pr-1])
    ...
    def get_user_item_pop(self):
        r"""
            get the user and item popularity
        """ 
        ...
        i_pop = torch.ones(self.item_num).long()
        i_pop[item_pop_id] = item_pop_cnt

        column = 'user_id'
        user_pop = torch.unique(interaction_data[column], return_counts=True)  # (arr(item_id), arr(count))
        user_pop_id = user_pop[0].tolist()
        user_pop_cnt = user_pop[1]

        u_pop = torch.zeros(self.user_num).long()
