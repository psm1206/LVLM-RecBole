...
def data_preparation_head_tail(config, dataset, verbose=False):
    ...
    if dataloaders is not None:
        ...
        # test_dataset을 popularity 기준으로 head, tail을 나누어서 dataloader를 만들어준다.
        ...
        head_indices = [i for i, test_item in enumerate(test_dataset.inter_feat['item_id']) if test_item in head_item]
        tail_indices = [i for i, test_item in enumerate(test_dataset.inter_feat['item_id']) if test_item in tail_item]
        ...
        head_test_data = get_dataloader(config, "test")(
            config, head_test_dataset, test_sampler, shuffle=False
        )

        tail_test_data = get_dataloader(config, "test")(
            config, tail_test_dataset, test_sampler, shuffle=False
        )

        if config["save_dataloaders"]:
            save_split_dataloaders(
                config, dataloaders=(train_data, valid_data, test_data, head_test_data, tail_test_data)
            )
