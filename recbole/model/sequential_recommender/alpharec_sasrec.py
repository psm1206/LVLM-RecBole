...
class AlphaRec_SASRec(SequentialRecommender):
    r"""
    AlphaRec is a sequential recommender that uses a self-attentive mechanism to model the sequential behavior of users.
    ...
    def __init__(self, config, dataset):
        super(AlphaRec_SASRec, self).__init__(config, dataset)
        ...
        self.adapter = nn.Sequential(
            nn.Linear(loaded_feat.shape[1], int(loaded_feat.shape[1] / 2)),
            nn.LeakyReLU(),
            nn.Linear(int(loaded_feat.shape[1] / 2), self.hidden_size))
    ...
    def item_embedding(self, item_seq):
        item_emb = self.llm_item_embedding(item_seq)
        item_emb = self.adapter(item_emb)
        return item_emb
