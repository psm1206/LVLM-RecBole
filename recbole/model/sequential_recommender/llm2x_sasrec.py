...
class LLM2X_SASRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        super(LLM2X_SASRec, self).__init__(config, dataset)
        ...
        print(f"[LLM2X_SASRec] PCA fit_transform started for input shape {mapped_feat[1:,:].shape}")
        if os.path.exists(f'{dataset_path}/pca_{config["text_encoder"]}_{config["dataset"]}.npy'):
            ...
            print(f"[LLM2X_SASRec] PCA fit_transform loaded from cache for input shape {mapped_feat[1:,:].shape}")
        else:
            ...
            print(f"[LLM2X_SASRec] PCA fit_transform took {_pca_elapsed:.3f}s for input shape {mapped_feat[1:,:].shape} and saved to cache")
