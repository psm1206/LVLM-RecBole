...
class LLM2X_CosSim_GRU4Rec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        super(LLM2X_CosSim_GRU4Rec, self).__init__(config, dataset)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == "BPR":
            ...
            test_item_emb = test_item_emb / (test_item_emb.norm(dim=-1, keepdim=True) + 1e-12)
            seq_output = seq_output / (seq_output.norm(dim=-1, keepdim=True) + 1e-12)
            logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
            # ce loss temperature scaling
    ...
    def full_sort_predict(self, interaction):
        ...
        # scaled cosine similarity
        test_items_emb = test_items_emb / (test_items_emb.norm(dim=-1, keepdim=True) + 1e-12)
        scores = torch.matmul(
